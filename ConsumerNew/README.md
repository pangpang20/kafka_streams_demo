...
| 配置目录 | 表名 | Topic | 质检 |
|----------|------|-------|------|
| config01 | baseinfo | baseinfo-topic | 启用 |
| config02 | orderinfo | orderinfo-topic | 跳过 |
...
```bash
# 启动 baseinfo 表消费者（config01，需要质检）
./start.sh --config-dir config01

# 启动 orderinfo 表消费者（config02，跳过质检）
./start.sh --config-dir config02

# 指定任务 ID 启动
./start.sh --config-dir config01 --task-id baseinfo-consumer-001
