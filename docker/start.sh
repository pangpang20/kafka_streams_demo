...
for i in {1..60}; do
    if docker exec kafka-1 bash -c 'kafka-broker-api-versions --bootstrap-server localhost:19091 --security-protocol SASL_PLAINTEXT --sasl-jaas-config "org.apache.kafka.common.security.plain.PlainLoginModule required username=\"admin\" password=\"Audaque@123\";"' &>/dev/null; then
        echo "  ✓ Kafka 集群已就绪"
        break
    fi
    if [ $i -eq 60 ]; then
...
echo "│  密码: Audaque@123                                          │"
echo "│                                                             │"
echo "│  SASL JAAS Config:                                          │"
echo "│  org.apache.kafka.common.security.plain.PlainLoginModule    │"
echo "│    required username=\"admin\" password=\"Audaque@123\";      │"
echo "│                                                             │"
echo "├─────────────────────────────────────────────────────────────┤"
echo "│                   Web UI 管理界面                            │"
echo "├─────────────────────────────────────────────────────────────┤"
...
echo "│      --command-config /etc/kafka/client.properties          │"
echo "│                                                             │"
...
echo "│      --producer.config /etc/kafka/client.properties         │"
echo "│                                                             │"
echo "│  消费消息:                                                  │"
echo "│    docker exec -it kafka-1 kafka-console-consumer           │"
echo "│      --bootstrap-server localhost:19091 --topic my-topic    │"
echo "│      --from-beginning                                       │"
echo "│      --consumer.config /etc/kafka/client.properties         │"
echo "│                                                             │"
echo "│  查看集群状态:                                              │"
